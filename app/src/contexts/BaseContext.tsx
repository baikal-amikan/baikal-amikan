...
export function BaseContextProvider({ children }: BaseContextProps) {
  ...
  useEffect(() => {
    ...
    if (!allSeasons || !season || !language) {
      getData(`/public/settings.json`).then((defaultSettings: {
  ...
  useEffect(() => {
    ...
    if (!allActivities) {
      getData(`/public/activities/all.json`).then((result: Array<string>) => {
        const activityPromises = result.map(activityId =>
          getData(`/public/activities/${activityId}/data.json`)
        );
  ...
  useEffect(() => {
    ...
    if (!allPlaces) {
      getData(`/public/places/all.json`).then((result: Array<string>) => {
        const placePromises = result.map(placeId =>
          getData(`/public/places/${placeId}/data.json`)
        );
  ...
  useEffect(() => {
    ...
    if (!allTours) {
      getData(`/public/tours/all.json`).then((result: Array<string>) => {
        const tourPromises = result.map(tourId =>
          getData(`/public/tours/${tourId}/data.json`)
        );
  ...
  useEffect(() => {
    ...
    if (!allArticles) {
      getData(`/public/articles/all.json`).then((result: Array<string>) => {
        const articlePromises = result.map(articleId =>
          getData(`/public/articles/${articleId}/data.json`)
        );
