      ...
      {language ?
        <>
          <div className={css.articlePreviewDiv}>
            ...
            {article.description ?
              <div className={css.articlePreviewText}>
                <Typography variant="h4" align="left" style={{ margin: "30px auto 5px auto" }}
          ...
          {version === "full" ? <>
            ...
            {article.text ? <Typography variant="body1" align="left"
                                        dangerouslySetInnerHTML={{ __html: article.text[language] }} /> : ""}
            {article.pictures?
              <PicturesBlock pictures={
                article.pictures.map((pic) => ({ title: pic.title[language], url: pic.src }))
              } />: ""}


            {article.video ? article.video.map((video, index) => (
              <div key={`video-${index}`} className={css.videoPreview}>
                {video.description?
                  <Typography variant="body1" align="left" style={{ margin: "0 auto 20px auto" }}
